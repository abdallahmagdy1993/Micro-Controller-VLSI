...
create_clock -name "ext_clock" -period $EXT_PERIOD [get_ports ext_clk]
set_clock_latency $CLOCK_LATENCY  [get_clocks ext_clock]
set_clock_uncertainty 0.3 [get_clocks ext_clock ]
set_clock_transition 0.4 [get_clocks ext_clock ]
...
#################################################################			
## IN/OUT
set INPUTPORTS [remove_from_collection [all_inputs] [get_ports {rst clk ext_clk} ]]
set OUTPUTPORTS [all_outputs]

set_driving_cell -lib_cell NBUFFX16 -pin Q [get_ports clk] -no_design_rule
set_driving_cell -lib_cell NBUFFX16 -pin Q [get_ports ext_clk] -no_design_rule
